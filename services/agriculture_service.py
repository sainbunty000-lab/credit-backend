...
def calculate_agri_logic(doc_inc, tax, undoc_m, emi_m):
    ...
    doc_inc = default_zero(doc_inc)
    tax = default_zero(tax)
    undoc_m = default_zero(undoc_m)
    emi_m = default_zero(emi_m)

    # Step 2: Net Documented Income
    net_doc = safe_subtract(doc_inc, tax)

    # Step 3: Annual Calculations
    ann_undoc = undoc_m * 12
    ann_emi = emi_m * 12

    # Step 4: Total Net Income
    total_net = safe_subtract(net_doc + ann_undoc, ann_emi)

    # Step 5: 5-Year Rule (14% factor)
