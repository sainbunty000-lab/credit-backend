...
def analyze_banking(transactions):
    ...
    total_credit, total_debit, bounce_count, emi_bounce_count = 0.0, 0.0, 0, 0
    fraud_flags, cash_credit_total = 0, 0.0
    credit_list = []
    ...
    for t in transactions:
        ...
        desc = str(t.get("description", "")).lower() # FIXED: aligned with parser
        date_str = t.get("date")
        ...
        if "return" in desc or "bounce" in desc:
            bounce_count += 1
            if "emi" in desc: emi_bounce_count += 1
        
        if "cash" in desc: cash_credit_total += credit
        if credit > 5000000: fraud_flags += 1
        if credit > 0: credit_list.append(credit)
    ...
    # Hygiene Score Logic
    hygiene_score = 100
    if net_monthly_surplus < 0: hygiene_score -= 30
    hygiene_score -= min(bounce_count * 5, 25)
    hygiene_score -= min(emi_bounce_count * 10, 20)
    if cash_ratio > 60: hygiene_score -= 10
    if concentration_ratio > 70: hygiene_score -= 10
    hygiene_score -= min(fraud_flags * 10, 20)
    hygiene_score = max(0, min(hygiene_score, 100))

    if hygiene_score >= 80: risk_grade, hygiene_status = "A", "Strong"
    elif hygiene_score >= 65: risk_grade, hygiene_status = "B", "Good"
    elif hygiene_score >= 50: risk_grade, hygiene_status = "C", "Moderate"
    else: risk_grade, hygiene_status = "D", "Weak"
    ...
    return {
        ...
        "chart_data": {
            "monthly_trend": [
                {"month": m, "credit": round(monthly_credit[m], 2), "debit": round(monthly_debit[m], 2)}
...
def extract_month(date_str):
    try:
        ...
        # FIXED: Handles DD/MM/YY from Camelot parser
